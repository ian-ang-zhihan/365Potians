...
def get_inventory():
    ...
    with db.engine.begin() as connection:
        ...
        potion_inventory = connection.execute(sqlalchemy.text("SELECT COALESCE(SUM(potion_change), 0) AS sum FROM potion_entries")).mappings().fetchone()
        ...
        gold_inventory = connection.execute(sqlalchemy.text("SELECT COALESCE(SUM(cha_change), 0) AS sum FROM cha_ching_entries")).mappings().fetchone()
